...
namespace Bugsnag.NetCore
{
    internal class NotificationFactory
    {
        ...
        public Notification CreateFromError(Event error)
        {
            ...
            RecursiveAddExceptionInfo(error, error.Exception, eventInfo);
            if (eventInfo == null)
                return null;
                                               ...
                                               //StackTrace lastStack = null)
        {
            ...
            //if (expInfo != null)
                currentEvent.Exceptions.Add(new ExceptionInfo
                {
                    ExceptionClass = exp.GetType().Name,
                    Description = exp.Message,
                    StackTrace = new List<StackTraceFrameInfo>
                    {
                        new StackTraceFrameInfo // .NET Core does not currently support Stack Frame APIs, supposedly in 2.0
                        {
                            LineNumber = 1,
                            Method = exp.StackTrace,
                            InProject = true
                        }
                    }
                });
            ...
            else
            {
                ...
                if (aggExp != null && aggExp.InnerExceptions.Count > 1)
                {
                    ...
                    {
                        RecursiveAddExceptionInfo(error, inner, currentEvent);
                    }
                }
                else
                {
                    // Otherwise just move to the next inner exception
                    RecursiveAddExceptionInfo(error, exp.InnerException, currentEvent);
                }
            }
        }
